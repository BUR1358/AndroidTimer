...
public class TimerView extends View {
    ...
    public void resume(float progress, int TIMER_LENGTH) {
        if (mTimerAnimator != null && mTimerAnimator.isPaused()) {
            stop();
            mTimerAnimator = ValueAnimator.ofFloat(1F - progress, 1f);
            mTimerAnimator.setDuration(TimeUnit.SECONDS.toMillis(TIMER_LENGTH));
            mTimerAnimator.setInterpolator(new LinearInterpolator());
            mTimerAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
